...
SELECT count(*) FROM  mv_daily_profit;
...
AS
SELECT product_name, cust.client_address, order_date,  sum(order_price) profit
...
as 
(
select t.product_name, t.order_date, t.order_price, t.order_id, 1 countt
from
...
SELECT DISTINCT product_name, order_date, extract(month FROM order_date) AS Month, order_price, order_id
...
dimension by (order_id, order_date)
...
SELECT count(*) FROM DW_DATA.mv_orders_daily;
